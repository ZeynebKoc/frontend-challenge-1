      ...
      <div class="flex flex-col gap-4 p-6 bg-white border border-gray-400 dark:border-gray-900 dark:bg-gray-900 rounded-lg min-w-[480px]">
        ...
        <div v-if="downloadStarted" class="flex flex-col gap-4">
          <div class="flex items-center gap-2">
            ...
            <h3 class="text-sm font-semibold text-black dark:text-white">Download Started</h3>
          </div>
        ...
        <template v-else>
          ...
          <div class="flex items-center justify-between">
            <div class="flex items-center gap-2">
              ...
              <h3 class="text-sm font-semibold text-black dark:text-white">Download Files</h3>
            </div>
            <UButton variant="ghost" color="neutral" size="xs" icon="i-heroicons-x-mark" @click="close" />
          </div>

          <div class="w-full h-px border-gray-300 dark:bg-gray-900" />

          <!-- File list -->
          <div class="flex flex-col border border-gray-400 dark:border-gray-700 rounded overflow-hidden max-h-80 overflow-y-auto">
            <div
              v-for="file in files"
              :key="file.fileId"
              class="flex items-center justify-between gap-4 px-3 py-2 border-b border-gray-300 dark:border-gray-800 last:border-0 hover:bg-gray-50 dark:hover:bg-gray-800"
            >
              <!-- File info -->
              <div class="flex flex-col gap-0.5 min-w-0">
                <span class="text-[11px] font-mono text-black dark:text-white truncate">
                  {{ file.fileName }}
                </span>
                <span class="text-[10px] text-gray-700 dark:text-gray-500">
